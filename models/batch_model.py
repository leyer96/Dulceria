...
class StockModel(QSqlQueryModel):
    ...
    def __init__(self, db):
        super().__init__()
        self.db = db
        self.headers = ["Producto", "Categoría", "Cantidad en Lote", "Fecha de Caducidad"]
        self.filter = None
    ...
    def search(self, search_str, filter):
        ...
        self.filter = filter
        query = """
            SELECT batch_test.product, product_test.category, batch_test.amount, batch_test.expiration_date FROM stock_test
